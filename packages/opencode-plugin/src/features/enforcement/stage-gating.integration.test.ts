...
function parseAuditEnvelope(result: ToolResult | string): {
  ...
  if (!match) {
    throw new Error(`goop_acceptance_audit did not return the expected JSON envelope: ${text}`);
  }
...
describe("stage-gated verification lifecycle (integration)", () => {
  ...
  it("2. in execute phase, the merged guard permits goop-wave-verifier and blocks goop-verifier", async () => {
    ...
    if (!guardHandler) throw new Error("merged tool.execute.before not defined");

    await expect(
      guardHandler({ tool: "task", sessionID: "s1", callID: "c1" }, taskArgs("goop-wave-verifier")),
    ).resolves.toBeUndefined();

    await expect(
      guardHandler({ tool: "task", sessionID: "s1", callID: "c2" }, taskArgs("goop-verifier")),
  ...
  it("4. a recorded fail verification row keeps completion blocked", async () => {
    ...
    const failRecorded = await writeWaveTool.execute(
      {
        wave_number: 1,
        verifications: [{ check_name: "test", status: "fail", detail: "3 failing specs" }],
      },
      toolCtx,
    );
    expect(failRecorded).toContain("Recorded test=fail verification for wave 1");

    const stillBlocked = await writeWaveTool.execute({ wave_number: 1, status: "done" }, toolCtx);
  ...
  it("5. remediation is represented without dispatching a subagent, and a later pass row for the same check unblocks completion", async () => {
    ...
    ctx.stateManager.appendADL({
      ...
      action: "Represents goop-executor-* remediation; re-verification recorded as a new row.",
    });
    expect(ctx.stateManager.getADL()).toContain("Executor remediation for wave 1");
    ...
    expect(passRecorded).toContain("Recorded test=pass verification for wave 1");

    const completed = await writeWaveTool.execute({ wave_number: 1, status: "done" }, toolCtx);
    ...
    if (!waveAfterCompletion) throw new Error("wave 1 not found after completion");
    ...
    expect(testRows.map((row) => row.status).sort()).toEqual(["failed", "passed"]);
  });
  ...
  it("6. final-wave completion with verification evidence permits execute → accept auto-progression", async () => {
    ...
    const afterHandler = autoProgressionHooks["tool.execute.after"] as NonNullable<
    ...
    expect(progressionOutput.output).toContain("Auto-Progression: execute → accept");
  });
  ...
  it("7. in accept phase, the merged guard permits goop-verifier and blocks goop-wave-verifier", async () => {
    ...
    await expect(
      guardHandler({ tool: "task", sessionID: "s1", callID: "c4" }, taskArgs("goop-verifier")),
    ).resolves.toBeUndefined();

    await expect(
      guardHandler({ tool: "task", sessionID: "s1", callID: "c5" }, taskArgs("goop-wave-verifier")),
    ).rejects.toThrow(IntentionalToolDenialError);
  });
  ...
  it("8. goop_acceptance_audit surfaces per-wave verification rows, including the append-only fail/pass history", async () => {
    ...
    // confirmation source.
    expect(parsed.waves).toContain("## Wave 1: Wave 1: stage-gated lifecycle proof");
