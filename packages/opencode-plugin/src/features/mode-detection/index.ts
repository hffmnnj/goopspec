...
export function detectTaskMode(prompt: string, options?: DetectorOptions): ModeDetectionResult {
  ...
  if (!normalized) {
    return {
      ...
      confidence: 0,
      reasoning: [`Empty prompt; defaulted to ${options?.defaultMode ?? DEFAULT_MODE}`],
      alternatives: [],
    };
  }
  ...
  const ranked = (Object.keys(scores) as TaskMode[]).sort((a, b) => scores[b] - scores[a]);
  ...
  const confidence = Math.max(0, Math.min(1, scoreConfidence * 0.6 + marginConfidence * 0.4));

  // 6. Alternatives within 0.2 of top score
  const alternatives = ranked.slice(1).filter((m) => topScore - scores[m] <= 0.2);
