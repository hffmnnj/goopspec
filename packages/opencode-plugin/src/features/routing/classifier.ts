...
function scoreCategory(category: RoutingCategory, text: string): CategoryScore {
  ...
  for (const signal of category.signals) {
    ...
    if (words.length > 1) {
      ...
      const pattern = words.map((w) => `\\b${escapeRegex(w)}\\b`).join("[\\s\\S]*?");
  ...
  if (category.antiSignals) {
    for (const anti of category.antiSignals) {
      ...
      if (antiWords.length > 1) {
        const pattern = antiWords.map((w) => `\\b${escapeRegex(w)}\\b`).join("[\\s\\S]*?");
        if (new RegExp(pattern, "i").test(text)) {
          antiMatches++;
        }
      } else {
...
export function classify(
  ...
  const scores: CategoryScore[] = ROUTING_CATEGORIES.map((cat) => scoreCategory(cat, text));
  ...
  if (!best || confidence < threshold) {
    return {
      ...
      tier: DEFAULT_TIER,
      confidence: 0,
      reason: "No category matched with sufficient confidence; defaulting to executor-high.",
      category: "fallback",
      matchedSignals: [],
    };
  }
