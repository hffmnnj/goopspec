...
function formatMatches(
	...
	lines.push(
		`Found ${matches.length} ${verb}${matches.length === 1 ? "" : "es"}:`,
	);
	lines.push("");
	...
	for (const [file, fileMatches] of byFile) {
		...
		for (const match of fileMatches) {
			...
			lines.push(
				`- [${start.line}:${start.column} -> ${end.line}:${end.column}] ${match.text}`,
			);
	...
	return tool({
		...
		args: {
			...
			rewrite: tool.schema
				...
				.optional()
				.describe(
					"Replacement string for rewrite mode (sets -r); omit for search mode",
				),
			apply: tool.schema
				.boolean()
				.optional()
		...
		async execute(
			...
			try {
				...
				const configured = loadMergedConfig(ctx.sdk.directory).binaryPaths?.[
					"ast-grep"
				];
				...
				const filteredCommandArgs = commandArgs.filter(
				...
				if (result.exitCode === 1) {
					return result.stdout.trim()
						? formatMatches(
								safeParseMatches(result.stdout),
								rewriteMode ? "rewrite" : "search",
							)
						: "No matches found.";
				}
