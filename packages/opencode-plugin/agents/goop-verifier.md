...
2. `goop_read_db({ doc_type: "spec" })` — must-haves and acceptance criteria.
3. `goop_read_db({ doc_type: "blueprint" })` — traceability and tasks.
4. `goop_read_db({ doc_type: "chronicle" })` — execution evidence.
