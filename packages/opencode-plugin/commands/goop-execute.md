...
Call `goop_state({ action: "get" })`. If `specLocked` is not `true` or `BLUEPRINT.md` is missing, return `BLOCKED` with:
...
1. Read `SPEC.md`, `BLUEPRINT.md`, `CHRONICLE.md`, and `PROJECT_KNOWLEDGE_BASE.md`.
2. For each wave:
