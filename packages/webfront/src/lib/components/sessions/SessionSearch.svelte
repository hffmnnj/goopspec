<script lang="ts">
  ...
  interface SessionSearchProps {
    ...
    value?: string;
    /** Called when the query changes (debounced handling is left to the caller). */
    onchange?: (query: string) => void;
    /** Optional ARIA label; defaults to "Search sessions". */
    label?: string;
  }

  let { value = $bindable(''), onchange, label = 'Search sessions' }: SessionSearchProps = $props();
