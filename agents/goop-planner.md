...
goop_reference({ name: "pr-creation" })           # Atomic PR conventions
```
...
Every BLUEPRINT.md you create MUST include a final task in the last wave titled **"Branch and PR"** (or similar). This task must:

- Create a focused, atomic PR for this specific feature — one PR per feature/workflow, not a bundled multi-feature PR
- Use `goop_create_pr` tool to enforce the terminology gate before opening the PR
- Write a PR title and body in plain English — no internal tooling terminology (no wave numbers, task IDs, agent names, or phase labels)
- Follow the branch naming convention from `pr-creation.md`: `feat/<name>`, `fix/<description>`, `chore/<description>`
- Reference `pr-creation.md` for the full PR body template (`## What / ## Why / ## How / ## Testing`)

### Terminology gate requirement:
The `goop_create_pr` tool scans title and body for forbidden terms before calling `gh pr create`. If the gate blocks, fix the violations and retry. Do not bypass the gate.

### Example task entry:
```markdown
### Task N.M: Branch and PR

**Wave:** N | **Parallel:** no | **Depends On:** all prior tasks complete

**Intent:** Open an atomic PR for this feature with a plain-English description.

**Deliverables:**
- [ ] PR opened via `goop_create_pr` with title and body passing the terminology gate

**Verification:**
goop_create_pr({ title: "...", body: "...", branch: "feat/...", base: "main" })

**Acceptance:**
- PR title uses conventional commit format with no internal terms
- PR body follows the What/Why/How/Testing template
- Terminology gate passes without errors
```
