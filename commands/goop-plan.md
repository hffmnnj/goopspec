...
5. **Spawn goop-planner** — With full discovery context and current depth setting (`shallow`/`standard`/`deep`). Instruct the planner to enforce atomic PR conventions: the blueprint must include a final "Branch and PR" task using `goop_create_pr` with a plain-English title and body following the atomic PR model from `pr-creation.md`.
...
- [ ] Blueprint includes a final "Branch and PR" task using `goop_create_pr` with plain-English description
