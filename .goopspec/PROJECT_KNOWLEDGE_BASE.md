...
- `src/hooks/system-transform.ts` — injects `<goopspec_state>`, `## PHASE ENFORCEMENT` rules (via `src/features/enforcement/phase-context.ts`), `<goopspec_memory>` (~800-token budget), `<goopspec_field_notes>` (~300 tokens, importance ≥ 8), and `<goopspec_db>` into every LLM call through `experimental.chat.system.transform`. Session caches with 30s TTL.
