...
- **`src/core/hooks-v2.ts`** — Registers V1 hook behavior with V2 runtime hooks (`ctx.session.hook("request", ...)` for system transforms, `ctx.tool.hook("execute.before/after", ...)` for tool lifecycle). V2 does not expose equivalents for config, chat-message, command, permission, event, or compaction hooks, so those remain V1-only. **Idle-prompt triage is V1-only as a consequence**: it captures the user prompt on `chat.message` and the V2 session `request` hook re-fires on every model call (not once per user message) with `messages: unknown[]` and no verified shape, so the prompt cannot be captured without guessing at a host-owned payload. The V2 adapter logs this limitation once at startup and the system-transform handler degrades to a no-op; see `agents/goop-orchestrator.md` §Idle-Prompt Triage.
